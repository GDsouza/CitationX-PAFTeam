...
var wx = "instrumentation/mfd/range-nm";
...
var etx = "instrumentation/mfd/etx";
...
var MFDDisplay = {
	new: func {
	  ...
	  m.canvas.addPlacement({"node": "screen_F"});
...
#      me.dme_enabled = 1; # electrical init
    ...
    setlistener("instrumentation/mfd/cdr-tot", func {
      me.showRect();
    },0,1);
    ...
    setlistener("/controls/flight/va", func {
	  ...
	  setlistener("systems/electrical/outputs/dme1",func(n) {
    ...
    me.DME();
      ...
      me.text.chrono.show();
		...
		me.text.wx.setText(sprintf("%2d",getprop(wx)));
  ...
  razMenu : func {
	...
	showRect: func {
		...
		foreach(var element;me.cdr) {
			if (getprop("instrumentation/mfd/cdr"~n)) {
