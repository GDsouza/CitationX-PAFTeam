...
var RMU = {
  ...
  init : func(x) {
    ...
    for (var i=0;i<12;i+=1) memVec[x].append(0);
	...
	listen : func(x) {
      ...
      ##### Electrical #####
		setlistener("systems/electrical/outputs/comm"~(x+1),func (n) {
      me.comm_enabled = n.getValue();
      me.display(x);
    },1,0);

		setlistener("systems/electrical/outputs/nav"~(x+1),func (n) {
      me.nav_enabled = n.getValue();
      me.display(x);
    },1,0);

		setlistener("systems/electrical/outputs/xpdr"~(x+1),func (n) {
      me.xpdr_enabled = n.getValue();
      me.display(x);
    },1,0);

		setlistener("systems/electrical/outputs/adf"~(x+1),func (n) {
      me.adf_enabled = n.getValue();
      me.display(x);
    },1,0);
      ...
      ##### ADF #####
		setlistener("instrumentation/adf["~x~"]/frequencies/selected-khz", func {
      me.display(x);
		},1,0);

		setlistener("instrumentation/adf["~x~"]/mode", func {
      me.display(x);
		},1,0);
    ...
    setlistener("instrumentation/transponder/unit["~x~"]/ident",func(n) {
      ...
      var id_timer = maketimer (0.3, func() {
        me.text.ident.setVisible(t==0 ? 1 : 0);
        t+=1;
        if (t==2) t=0;
  ...
  display : func(x) {
    if (me.comm_enabled) {
      ...
      me.text.comFreq.setText("-----");
      me.text.comStby.setText("-----");
    }
    if (me.nav_enabled) {
	    me.text.navFreq.setText(sprintf("%.3f",getprop("instrumentation/nav["~x~"]/frequencies/selected-mhz")));
	    me.text.navStby.setText(sprintf("%.3f",getprop("instrumentation/nav["~x~"]/frequencies/standby-mhz")));
    } else {
      me.text.navFreq.setText("-----");
      me.text.navStby.setText("-----");
    }
    if (me.xpdr_enabled) {
      ...
      me.text.trspCode.setText("-----");
      me.text.trspNum.setText("-");
  		me.text.trspMode.setText("----");
    }
    if (me.adf_enabled) {
...
#		me.text.adfFreq.setText(sprintf("%d",getprop("instrumentation/adf["~x~"]/frequencies/selected-khz")));
	...
	update : func(x) { ### Memories runtime update ###
		me.timer = maketimer(0.1,func() {
			...
			if (memVec[x].vector[6]==0) setprop("instrumentation/rmu/unit["~x~"]/more",0);
			me.mem_redraw(x);
  ...
  dmeDisplay : func(x,dt) {
    if (me.dme_enabled) {
      ...
      navaid = findNavaidByFrequencyMHz(freq);
      if (navaid != nil) {
      ...
      me.dmeColor(x,dt);
        ...
        me.text.dmeFreq.setText("-----");
        me.text.dmeId.setText("-----");
    }
  }, # end of dmeDisplay
