...
var altClosed = "autopilot/route-manager/alternate[1]/closed";
var altDestApt = "autopilot/route-manager/alternate[1]/airport";
var altDestRwy = "autopilot/route-manager/alternate[1]/runway";
var altFlag = "autopilot/route-manager/alternate[1]/set-flag";
var cduDisplay0 = "instrumentation/cdu/display";
var cduInput0 = "instrumentation/cdu/input";
...
var direct = "instrumentation/cdu[1]/direct";
var direct_to = "instrumentation/cdu[1]/direct-to";
var fp_active = "autopilot/route-manager/active";
var nbPage = "instrumentation/cdu[1]/nbpage";
...
var cduMain1 = {
	new: func () {
		var m = {parents:[cduMain1]};
    return m;
  }, # end of new

  listen : func {
    setlistener(posit, func(n) { ### End of Positionning ###
	    if (n.getValue()) {setprop("instrumentation/cdu[1]/input","")}
    },0,0);

    setlistener("instrumentation/cdu[1]/display",func(n) {
  ...
  btn : func (v) { ### Alphanumeric Buttons treatment
	  var n = size(getprop("/instrumentation/cdu[1]/input"));
		  if (n < 13) {
			  setprop("/instrumentation/cdu[1]/input",getprop("/instrumentation/cdu[1]/input")~v);
		  }
  }, # end of btn

  key : func(v) { ### Keys treatment
    cduInput = getprop("/instrumentation/cdu[1]/input");	
    cduDisplay = getprop("/instrumentation/cdu[1]/display");
    ...
    if (v=="PERF" and getprop(pos_init)) {v="";cduDisplay = "PRF-PAGE[1]"}
      ...
      if (v == "B4R"){
        ...
        cduDisplay = "POS-INIT[1]";
    		setprop(cduDisplay0,cduDisplay);
      }
		}
        ...
        me.lineSelect(v);
				if (getprop("instrumentation/cdu[1]/"~select) !="") {
          navSel = getprop("instrumentation/cdu[1]/"~select);
        ...
        me.lineSelect(v);
				if (getprop("instrumentation/cdu[1]/"~select) !="") {
					setprop(depRwy,getprop("instrumentation/cdu[1]/"~select));
        ...
        if (!getprop(fp_active)) {
          me.lineSelect(v);
				  if (getprop("instrumentation/cdu[1]/"~select) !="") {
					  var SidName = getprop("instrumentation/cdu[1]/"~select);
					  fp.sid = SidName;
        ...
        me.lineSelect(v);
				if (getprop("instrumentation/cdu[1]/"~select) !="") {
          if (getprop(altFlag)) {
            setprop(altDestRwy,getprop("instrumentation/cdu[1]/"~select));
          } else {
  					setprop(destRwy,getprop("instrumentation/cdu[1]/"~select));
          }
					cduInput = "RWY "~getprop("instrumentation/cdu[1]/"~select)~" Loaded";
				}
			}
		}
        ...
        if (!getprop(fp_active)) {
          me.lineSelect(v);
				  if (getprop("instrumentation/cdu[1]/"~select) !="") {
					  var StarName = getprop("instrumentation/cdu[1]/"~select);
					  fp.star = StarName;
        ...
        if (!getprop(fp_active)) {
          me.lineSelect(v);
				  if (getprop("instrumentation/cdu[1]/"~select) !="") {			
					  var ApprName = getprop("instrumentation/cdu[1]/"~select);
            var n = 99;
...
#          setprop("autopilot/route-manager/flight-plan",getprop(depAirport)~"-"~getprop(destAirport));
        ...
        ind = nrPage*3-(3-substr(v,1,1))-1;
            ...
            if (size(cduInput) < 2) {cduInput = "0"~cduInput}
...
#              fp.insertWP(altFp.getWP(i),wpCurr+i);
            ...
            setprop(direct,0);
            setprop(direct_to,-1);
        		setprop("instrumentation/cdu[1]/display","FLT-PLAN[1]");
...
#			if (v == "B3R") {v = "";cduDisplay = "NAV-ARRV[1]"}
      ...
      if (v) {
        ...
        if (select) {
			    if (getprop("instrumentation/cdu[1]/"~select) !="") {
            cduInput = getprop("instrumentation/cdu[1]/"~select);
            navSel = cduInput;
          }
        }
      }
      ...
      cduInput = "";
        ...
        setprop("/instrumentation/cdu[1]/display","PRF-PAGE[1]");
				cduInput = "";
			}
      ...
      setprop("/instrumentation/cdu[1]/display","PRF-PAGE[2]");      
      ...
      setprop("/instrumentation/cdu[1]/display","PRF-PAGE[3]");      
      ...
      setprop("/instrumentation/cdu[1]/display","PRF-PAGE[4]");      
      ...
      setprop("/instrumentation/cdu[1]/display","PRF-PAGE[5]");      
			if (v == "B4R"){
				v = "";
        ...
        me.lineSelect(v);
			  if (getprop("instrumentation/cdu[1]/"~select) !="") {
          var freq_sel = getprop("instrumentation/cdu[1]/"~select);
  ...
  insertWayp : func(ind,cduInput,fp) {
    ...
    var wp_spd = fp.getWP(ind).speed_cstr;
      ...
      setprop("instrumentation/cdu[1]/speed",1);
    } else { ### Altitude
        if (getprop(altFlag)) {
  ...
  delete : func {
		var length = size(getprop("instrumentation/cdu[1]/input")) - 1;
		setprop("instrumentation/cdu[1]/input",substr(getprop("/instrumentation/cdu[1]/input"),0,length));
		if (length == -1 ) {
			setprop("instrumentation/cdu[1]/input","DELETE");
		}
  }, # end of delete
  ...
  previous : func {
    ...
    nrPage = size(cduDisplay)<12 ? substr(cduDisplay,9,1) : substr(cduDisplay,9,2); 
	  if (nrPage > 1) {
		  nrPage -= 1;
		  setprop("/instrumentation/cdu[1]/display",left(cduDisplay,8)~"["~nrPage~"]");
	  }
  }, # end of previous
  ...
  next_key : func {
    ...
    nrPage = size(cduDisplay)<12 ? substr(cduDisplay,9,1) : substr(cduDisplay,9,2); 
      ...
      if (getprop(depAirport) == "") {
   		  setprop("/instrumentation/cdu[1]/input", "NO DEP AIRPORT");
      } else if (getprop(destAirport) == "") {
   		  setprop("/instrumentation/cdu[1]/input", "NO DEST AIRPORT");
      }
    } else if (left(cduDisplay,8) == "FLT-PLAN" and nrPage < getprop(nbPage)) {
      if (getprop(destAirport) == "") {
	      setprop("/instrumentation/cdu[1]/input", "NO DEST AIRPORT");
    	} else if (getprop(destRwy) == "") {
	      setprop("/instrumentation/cdu[1]/input", "NO DEST RUNWAY");
      } else {
          nrPage += 1;
          setprop("/instrumentation/cdu[1]/display",left(cduDisplay,8)~"["~nrPage~"]");
      }
