...
var getfreqs = func(apt) { # apt may be an airportinfo() ghost, an ICAO or any other parm accepted by airportinfo() .
    ...
    if(size(comms)>0) {
        ...
        foreach (var hash; comms) {
            var typ = hash.ident;
            if(hash["TWR"]==nil and (string.match(hash.ident,"*TWR") or string.match(hash.ident,"*ower") or (string.match(hash.ident,"*MULT") and !string.match(hash.ident,"*LIGHTS*")))) typ ="TWR";
