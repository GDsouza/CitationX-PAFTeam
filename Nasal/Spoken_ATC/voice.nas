...
var window = screen.window.new(nil,-50,10,10);
var YELLOW = [1, .9, 0, 1];
var GREEN = [0, 1, 0, 1];
...
var speak = func(secs=0) {
  ...
  if(getprop("/devices/status/keyboard/alt") and getprop("/devices/status/keyboard/ctrl")) var closest=1; # Vector to closest airport
  else var closest=0;
  ...
  var commN = getprop("/satc/comm2")? '/instrumentation/comm[1]' : '/instrumentation/comm[0]';
  var tunned =  getprop(sprintf("%s/frequencies/selected-mhz", commN));
  ...
  var station = getprop(sprintf("%s/station-type", commN)); 
  var icao = getprop(sprintf("%s/airport-id", commN));
  ...
  # 2) Get properties
  var aux1 = getprop(sprintf("%s/station-name", commN));
  ...
  var atcname = sprintf("%s %s, ", string.replace(aux,"International",""),station );
  getfreqs(info);
...
#  fcrse = geo.aircraft_position().course_to(geofinal);
  ...
  # 5.5) Update props
  setprop("/satc/callsign-fmt", " "~alpha(substr(getprop("/sim/multiplay/callsign"),-3) ));
  ...
  if( closest){
    ...
    settimer(func(){speach("atc",msg,secs,YELLOW);}, 1);
    return;
  }
...
#  print("atcinstr="~atcinstr); 
  ...
  var timer0 = maketimer(delayed, func(){speach("atc",instruction,secs,YELLOW);});
  ...
  if(choosed=="land") {
    ...
    var timer = maketimer(5, func(){
      if((getprop("/position/altitude-agl-ft")<30)){
        speach("atc",mycs~join("exitrwy"),secs,YELLOW);
  ...
  if(choosed=="takeoff") {
    ...
    var timer2 = maketimer(10, func(){
      if(getprop("/position/altitude-agl-ft")>400){
        speach("atc",mycs~join("bye"),secs,YELLOW);
  ...
  var timer1 = maketimer(delayed+1+.09*size(instruction), func(){acknowledge(choosed,mycs,q) });
