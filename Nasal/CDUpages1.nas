...
var cduInput = "instrumentation/cdu[1]/input";
...
var spd = "instrumentation/cdu[1]/speed";
...
var cduDsp1 = {
	new: func () {
		...
		m.canvas.addPlacement({"node": "CDU.screenR"});
  ...
  Listen : func {
    ...
    setlistener(spd, func(n) {
      setprop("instrumentation/cdu[1]/speed",n.getValue());
      me.Flp1();
    },0,1);
...
#    me.fp = flightplan();
    ...
    p = 0;
	  for(var i=0;i<fp_size;i+=1) {		
          ...
          if (left(me.fp.getWP(i).wp_name,4) != me.dest_apt or me.fp_closed) {
            ...
            setprop("instrumentation/cdu[1]/l6",me.fp.getWP(i).wp_name);
          }
          me.line.r5.setText("");
    ...
    if (me.nrPage <= getprop(nbPage)) {
       if (n != nil and n < 3 ) {
        ...
        me.line.r6r.setText(getprop(dest_apt)~" "~getprop(dest_rwy))
                   .setColor(me.green);
        setprop("instrumentation/cdu[1]/r5","DEST");
        setprop("instrumentation/cdu[1]/r6",getprop(dest_apt));
      }
    }
  ...
  Dept : func {
	  ...
	  cdu1.cduMain1.nb_pages(size(dep_rwy),6);				
    me.nrPage = size(getprop(display))<12 ? substr(getprop(display),9,1) : substr(getprop(display),9,2); 
	  if (size(dep_rwy) == 0) {setprop("instrumentation/cdu[1]/input","*NO FILE*")}
    me.Raz_lines();
  ...
  Sid : func {
	  ...
	  if (size(xfile) == 0) {setprop("instrumentation/cdu[1]/input","*NO FILE*")}
    me.Raz_lines();
	  cdu1.cduMain1.nb_pages(size(xfile),6);				
    me.nrPage = size(getprop(display))<12 ? substr(getprop(display),9,1) : substr(getprop(display),9,2); 
  ...
  Arwy : func {
    ...
    if (getprop(altFlag)) {
	  ...
	  cdu1.cduMain1.nb_pages(size(apt_rwy),6);				
    me.nrPage = size(getprop(display))<12 ? substr(getprop(display),9,1) : substr(getprop(display),9,2); 
	  if (size(apt_rwy) == 0) {setprop("instrumentation/cdu[1]/input","*NO FILE*")}
  ...
  Nav_sel : func {
    me.nrPage = substr(getprop(display),9,1);
    cdu_ret = cdu1.cduMain1.nav_var();
    navSel = cdu_ret[0];
    if (me.nrPage == 1 ) {
      me.Raz_lines();
	    var flp_sel = getprop("instrumentation/cdu[1]/input");
    ...
    if (me.nrPage == 1 ) {
      ...
      me.line.r7.setText("NAV 2 >");
        ...
        p = 0;
            ...
            setprop("instrumentation/cdu[1]/l"~(p+1),sprintf("%.2f",ind.frequency/100));
		        p+=1;
          }
	      }
  ...
  Dsp_files : func(xfile) {
    p = 0;
    for (i=1;i<7;i+=1) {setprop("instrumentation/cdu[1]/l"~i,"")} # raz
	    ...
	    p+=1;
      if (n >= 0 and n < 6) {
        setprop("instrumentation/cdu[1]/l"~(n+1),file)
      }
	  }
  }, # end of Dsp_files
