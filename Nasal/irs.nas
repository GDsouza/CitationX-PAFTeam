...
var IRS = {
    ...
    if (!x) {
		  ...
		  canvas.parsesvg(m.irs,"Models/Instruments/IRS/irs.svg");
    } else {
		  m.canvas = canvas.new({
			  "name": "IRS2", 
			  "size": [1024, 1024],
    ...
    setlistener(stbyBatt_fail, func(n) {
      if (n.getValue()) me.text.BattFail.show();
		...
		setlistener(cdu_dsp[x], func(n) {
			if (n.getValue() == "POS-INIT" and !getprop(posit[x])) me.align(x);
		},0,0);
      ...
      if (n.getValue() == -2) {
		...
		setlistener(test[x], func(n) {
			if (n.getValue()) {
				...
				if (!getprop(align[x])) me.text.Align.hide();
